...
import './calender.css'
import { v4 } from 'uuid'
import { addMonths, endOfMonth, startOfMonth,subMonths, getDaysInMonth } from 'date-fns'
...
const Calender = ({ setDateRange }) => {
  ...
  const [week, setWeek] = useState({
    firstDay: startOfMonth(new Date(), { weekStartsOn: 1 }),
    lastDay: endOfMonth(new Date(), { weekStartsOn: 1 }),
  })
  ...
  const handleClick = (e) => {
    ...
    const firstDay = startOfMonth(localDate, { weekStartsOn: 1 })
    const lastDay = endOfMonth(localDate, { weekStartsOn: 1 })
